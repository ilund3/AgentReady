...
import { useToast } from "@inspector/lib/hooks/useToast";
...
} from "@inspector/utils/configUtils";
import { getMCPServerRequestTimeout } from "@inspector/utils/configUtils";
import { InspectorConfig } from "../configurationTypes";
import { Transport } from "@modelcontextprotocol/sdk/shared/transport.js";
import { CustomHeaders } from "../types/customHeaders";
import { resolveRefsInMessage } from "@inspector/utils/schemaUtils";
  ...
  const connect = async (_e?: unknown, retryCount: number = 0) => {
    ...
    try {
          ...
          case "streamable-http":
            transportOptions = {
              ...
              fetch: async (
                ...
                // Accept always; Content-Type only when there's a body (avoids CORS preflight on GET)
                ...
                const response = await fetch(url, {
                  ...init,
                  headers: { ...reqHeaders, ...(init?.headers as Record<string, string> | undefined) },
                });
              ...
              requestInit: {
                headers: { ...requestHeaders, Accept: "text/event-stream, application/json" },
              },
      ...
      const connectionTimeoutPromise = new Promise<never>((_, reject) => {
        timeoutId = setTimeout(
          () =>
            reject(
              new Error(
                "Connection timed out. Check the URL, CORS (for Direct), and that the server is an MCP SSE/HTTP endpoint.",
              ),
            ),
      ...
      try {
        ...
        await Promise.race([
          client.connect(transport as Transport).finally(() => {
        ...
        const isCorsOrNetwork =
          /failed to fetch|network error|cors/i.test(message) ||
          (error instanceof TypeError && message.includes("fetch"));
        toast({
          title: "Connection failed",
          description: isCorsOrNetwork
            ? `${message}. If using Direct connection, ensure the MCP server allows your origin (CORS).`
            : message,
          variant: "destructive",
        });
        throw error;
