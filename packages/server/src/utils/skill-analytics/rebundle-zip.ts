...
export function buildTelemetryShimJs(_pluginId: string): string {
	// Split into small string segments and join to avoid TypeScript 5.9.x escapeString
	// stack-overflow (tsc emit crashes on very large template literals).
	const lines: string[] = [
		'"use strict";',
		'var fs = require("fs");',
		'var path = require("path");',
		'var DEBUG = process.env.AGENTREADY_ANALYTICS_DEBUG === "1" || process.env.AGENTREADY_ANALYTICS_DEBUG === "true";',
		"",
		"function loadConfig() {",
		"  try {",
		'    var p = path.join(__dirname, "agentready-analytics.json");',
